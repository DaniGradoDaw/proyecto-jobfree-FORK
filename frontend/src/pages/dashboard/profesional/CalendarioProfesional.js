...
const ESTADO_META = {
  ...
  CONFIRMADA: { color: "bg-emerald-100 text-emerald-700 ring-emerald-200", dot: "bg-emerald-500", evento: "#10b981", label: "Aceptada" },
...
const FILTROS = ["todas", "PENDIENTE", "CONFIRMADA", "COMPLETADA"];
  ...
  return (
    <div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { estado: "PENDIENTE",  label: tx("Pendientes"),  bg: "bg-amber-50",   text: "text-amber-700",   border: "border-amber-100" },
          { estado: "CONFIRMADA", label: tx("En curso"),    bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-100" },
        ...
        ].map(({ estado, label, bg, text, border }) => (
          <button
            key={estado}
            onClick={() => setFiltro(filtro === estado ? "todas" : estado)}
            className={`rounded-xl border ${border} ${bg} p-3 text-center transition hover:opacity-80 ${filtro === estado ? "ring-2 ring-offset-1 ring-current" : ""}`}
          >
